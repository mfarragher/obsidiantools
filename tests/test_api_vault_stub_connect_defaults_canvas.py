...
def test_canvas_file_index(actual_connected_vault, expected_canvas_file_index):
    assert actual_connected_vault.canvas_file_index == expected_canvas_file_index


def test_canvas_content_index(actual_connected_vault):
    actual_crazy_wall_2_content = actual_connected_vault.canvas_content_index.get(
        "Crazy wall 2.canvas"
    )
    actual_crazy_wall_2_text = actual_crazy_wall_2_content.get("nodes")[0].get("text")
...
def test_canvas_graph_detail_index_graph(actual_connected_vault):
    actual_crazy_wall_graph_detail = (
        actual_connected_vault.canvas_graph_detail_index.get("Crazy wall.canvas")
    )
    ...
    assert not {"a10aaf054261706d"}.issubset(set(actual_nodes))

    # isolated nodes:
    actual_isolated_nodes = list(nx.isolates(G))
...
def test_canvas_graph_detail_index_graph_other_attributes(actual_connected_vault):
    ...
    actual_non_blank_edge_labels = {
        pair: label for pair, label in edge_labels.items() if label != ""
    }
