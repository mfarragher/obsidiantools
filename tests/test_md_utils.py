...
from obsidiantools.md_utils import (
...
def test_get_all_embedded_files_from_source_text(html_wikilinks_stub):
    actual_results = _get_all_embedded_files_from_source_text(html_wikilinks_stub)
    expected_results = ["Egg.jpg", "Easter egg.png", "Egg.jpg"]

    assert actual_results == expected_results


def test_get_all_embedded_files_from_source_text_keep_aliases(html_wikilinks_stub):
...
def test_sussudio_front_matter():
    ...
    actual_metadata = get_front_matter(Path(".") / "tests/vault-stub/Sussudio.md")
    assert actual_metadata == expected_metadata


def test_ne_fuit_front_matter():
    expected_metadata = {}
    actual_metadata = get_front_matter(Path(".") / "tests/vault-stub/lipsum/Ne fuit.md")
    assert actual_metadata == expected_metadata
...
def test_sussudio_tags_with_nesting_not_shown():
    actual_tags = get_tags(Path(".") / "tests/vault-stub/Sussudio.md")
    expected_tags = ["y1982", "y_1982", "y-1982", "y1982", "y2000"]
    assert actual_tags == expected_tags


def test_sussudio_tags_with_nesting_shown():
    actual_tags = get_tags(Path(".") / "tests/vault-stub/Sussudio.md", show_nested=True)
    expected_tags = [
        "y1982",
        "y_1982",
        "y-1982",
...
def test_wikilinks_code_block():
    actual_links = get_wikilinks(Path(".") / "tests/general/wikilinks_exclude-code.md")
    expected_links = []
    assert actual_links == expected_links
...
def test_remove_wikilinks(txt_wikilink_extraction_stub):
    out_str = _remove_wikilinks_from_source_text(txt_wikilink_extraction_stub)
...
def test_wikilinks_as_readable_text(txt_wikilink_extraction_stub):
    out_str = _replace_wikilinks_with_their_text(txt_wikilink_extraction_stub)
...
def test_md_links_as_readable_text(txt_md_link_extraction_stub):
    out_str = _replace_md_links_with_their_text(txt_md_link_extraction_stub)
...
def test_readable_text_from_latex_md_stub_default_tags():
    actual_str = get_readable_text_from_md_file(Path(".") / "tests/general/latex.md")
...
def test_source_text_has_text_after_problematic_latex():
    expected_str = """# Generalised estimating equations
...
"""
    ...
    actual_str = get_source_text_from_md_file(
        Path(".") / "tests/general/source-text_handle-latex.md", remove_math=True
    )
