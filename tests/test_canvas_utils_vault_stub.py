...
def test_get_canvas_relpaths_matching_subdirs(actual_vault_path):
    ...
    expected_in_root = [Path("Crazy wall.canvas"), Path("Crazy wall 2.canvas")]
    assert set(actual_w_root).difference(actual_wo_root) == set(expected_in_root)
    ...
    assert set(actual_w_lipsum_only).difference(actual_wo_root) == set(expected_in_root)

    # both kwargs not default:
    actual_w_lipsum_only = get_canvas_relpaths_matching_subdirs(
        actual_vault_path, include_subdirs=["lipsum"], include_root=False
    )
    assert set(actual_w_lipsum_only).difference(actual_wo_root) == set()
