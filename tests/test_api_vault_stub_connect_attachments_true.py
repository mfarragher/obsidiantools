...
def actual_connected_vault():
    return Vault(WKD / "tests/vault-stub").connect(attachments=True)
...
def expected_media_file_metadata_dict():
    return {
        "rel_filepath": {"1999.flac": np.nan, "Sussudio.mp3": np.nan},
        # abs_filepath would be here
        "file_exists": {"1999.flac": False, "Sussudio.mp3": False},
        "n_backlinks": {"1999.flac": 1, "Sussudio.mp3": 1},
    }
...
def test_get_metadata_backlinks(actual_note_metadata_df, expected_note_metadata_dict):
    TEST_COL = "n_backlinks"

    actual_series = actual_note_metadata_df[TEST_COL]
    expected_series = pd.Series(
        expected_note_metadata_dict.get(TEST_COL), name=TEST_COL
    ).rename_axis("note")
...
def test_backlink_and_wikilink_totals_not_equal_for_test_vault(actual_note_metadata_df):
    ...
    # for this vault
    assert (
        actual_note_metadata_df["n_backlinks"].sum()
...
def test_get_canvas_file_dicts_tuple(actual_connected_vault):
    ...
    actual_tuple = actual_connected_vault._get_canvas_file_dicts_tuple()
    expected_tuple = (
...
def test_get_media_file_dicts_tuple(actual_connected_vault):
    ...
    #  nonexistent_files_by_short_path)
    actual_tuple = actual_connected_vault._get_media_file_dicts_tuple()
    expected_tuple = ({}, {}, {"Sussudio.mp3": np.nan, "1999.flac": np.nan})
    assert actual_tuple == expected_tuple


def test_nonexistent_canvas_files(
    actual_connected_vault, actual_canvas_file_metadata_df
):
    expected_non_e_files = []

    assert isinstance(actual_connected_vault.nonexistent_canvas_files, list)

    assert set(actual_connected_vault.nonexistent_canvas_files) == set(
        expected_non_e_files
    )
    assert set(
        actual_canvas_file_metadata_df.loc[
            ~actual_canvas_file_metadata_df["file_exists"], :
        ].index.tolist()
...
def test_nonexistent_media_files(actual_connected_vault, actual_media_file_metadata_df):
    ...
    assert set(actual_connected_vault.nonexistent_media_files) == set(
        expected_non_e_files
    )
    assert set(
        actual_media_file_metadata_df.loc[
            ~actual_media_file_metadata_df["file_exists"], :
        ].index.tolist()
    ) == set(expected_non_e_files)
...
def test_isolated_canvas_files(actual_connected_vault):
    ...
    assert set(actual_connected_vault.isolated_canvas_files) == set(expected_isol_files)
...
def test_all_file_metadata_df(actual_connected_vault):
    ...
    assert len(actual_all_df) == (
        len(actual_notes_df) + len(actual_media_df) + len(actual_canvas_df)
    )
    ...
    assert actual_all_df["n_backlinks"].sum() == (
        actual_all_df["n_wikilinks"].sum() + actual_all_df["n_embedded_files"].sum()
    )
