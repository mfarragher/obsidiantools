...
def test_get_metadata_rel_filepath(actual_metadata_df, expected_metadata_dict):
    TEST_COL = "rel_filepath"

    actual_series = actual_metadata_df[TEST_COL]
    expected_series = pd.Series(
        expected_metadata_dict.get(TEST_COL), name=TEST_COL
    ).rename_axis("note")
    assert_series_equal(actual_series, expected_series, check_like=True)


def test_get_metadata_note_exists(actual_metadata_df, expected_metadata_dict):
...
def test_get_metadata_wikilinks(actual_metadata_df, expected_metadata_dict):
...
def test_get_metadata_backlinks(actual_metadata_df, expected_metadata_dict):
...
def test_get_metadata_tags(actual_metadata_df, expected_metadata_dict):
    TEST_COL = "n_tags"
...
def test_backlink_and_wikilink_totals_equal(actual_metadata_df):
    ...
    assert (
        actual_metadata_df["n_backlinks"].sum()
...
def test_backlink_individual_notes(actual_connected_vault):
    ...
    assert len(actual_bl_ix) == (actual_connected_vault.graph.number_of_nodes())
    for k in list(expected_bl_subset.keys()):
...
def test_backlink_counts(actual_connected_vault):
    expected_bl_count_subset = {
        "Sussudio": {},
        "Alimenta": {},
        "Tarpeia": {"Brevissimus moenia": 1, "Alimenta": 1, "Vulnera ubera": 1},
        "Ne fuit": {"Alimenta": 1, "Causam mihi": 1},
        "Bacchus": {"Ne fuit": 1, "Alimenta": 4},
    }
...
def test_wikilink_counts(actual_connected_vault):
    expected_wl_count_subset = {
        ...
        "Isolated note": {},
        "Vulnera ubera": {"Caelum": 1, "Tarpeia": 1, "Vita": 1},
        "Ne fuit": {
            "Aras Teucras": 1,
            "Manus": 1,
            "Bacchus": 1,
        ...
        "Causam mihi": {"Dives": 1, "Manus": 1, "Ne fuit": 1, "Aetna": 1},
        "Brevissimus moenia": {"Tarpeia": 1, "Caelum": 1, "Vita": 1},
        "Alimenta": {
            "Manus": 1,
            "Bacchus": 4,
            "Amor": 1,
    ...
    for k in list(expected_wl_count_subset.keys()):
        assert actual_connected_vault.get_wikilink_counts(
            k
...
def test_wikilink_individual_notes(actual_connected_vault):
    ...
    assert actual_connected_vault.get_wikilinks("Alimenta") == expected_wl_subset.get(
        "Alimenta"
    )
    ...
    for k in list(actual_wl_ix.keys()):
        assert isinstance(actual_connected_vault.md_file_index.get(k), Path)
...
def test_unique_wikilinks(actual_connected_vault):
    ...
    assert actual_u_wl_ix.get("Alimenta") == expected_u_wl_subset.get("Alimenta")


def test_nonexistent_notes(actual_connected_vault, actual_metadata_df):
    expected_non_e_notes = [
        "Tarpeia",
        "Caelum",
        "Vita",
        "Aras Teucras",
    ...
    assert set(actual_connected_vault.nonexistent_notes) == set(expected_non_e_notes)
    assert set(
        actual_metadata_df.loc[~actual_metadata_df["note_exists"], :].index.tolist()
...
def test_isolated_notes(actual_connected_vault):
    ...
    assert set(actual_connected_vault.isolated_notes) == set(expected_isol_notes)
...
def test_front_matter_index(actual_connected_vault, expected_front_matter_index):
...
def test_embedded_files_index(actual_connected_vault, expected_embedded_files_index):
    actual_files_ix = actual_connected_vault.embedded_files_index
    assert actual_files_ix == expected_embedded_files_index


def test_math_index(actual_connected_vault, expected_math_index):
    actual_math_index = actual_connected_vault.math_index
    assert actual_math_index == expected_math_index


def test_md_links_index(actual_connected_vault, expected_md_links_index):
    actual_md_links_ix = actual_connected_vault.md_links_index
    assert actual_md_links_ix == expected_md_links_index


def test_tags_index(actual_connected_vault, expected_tags_main_only_index):
    actual_tags_ix = actual_connected_vault.tags_index
    assert actual_tags_ix == expected_tags_main_only_index


def test_unique_md_links(actual_connected_vault, expected_md_links_index):
    actual_u_md_links_ix = actual_connected_vault._unique_md_links_index
...
def test_media_file_metadata_df_empty(actual_connected_vault_md_files_only):
    ...
    df_media = actual_connected_vault_md_files_only.get_media_file_metadata()
...
def test_canvas_file_metadata_df_empty(actual_connected_vault_md_files_only):
    # use the lipsum dir as the 'vault' dir (md only)
    df_media = actual_connected_vault_md_files_only.get_canvas_file_metadata()
...
def test_all_file_metadata_df(actual_connected_vault):
    ...
    assert actual_all_df["n_backlinks"].sum() == (actual_all_df["n_wikilinks"].sum())


def test_internal_canvas_backlink_counts_func_errors(actual_connected_vault):
    with pytest.raises(AttributeError):
        (actual_connected_vault._get_backlink_counts_for_canvas_files_only())
